...
export class DeckGLMap {
  ...
  private maybeStartAutoRotate(): void {
    ...
    const step = (ts: number): void => {
      ...
      if (!this.maplibreMap || this.state.mode !== '3d') return;
      if (this.userInteracting || this.renderPaused || this.webglLost || document.hidden) {
      ...
      const dt = (ts - this.autoRotateLastTs) / 1000;
      this.autoRotateLastTs = ts;
      const center = this.maplibreMap.getCenter();
      const nextLng = ((center.lng + DeckGLMap.AUTO_ROTATE_DEG_PER_SEC * dt + 540) % 360) - 180;
