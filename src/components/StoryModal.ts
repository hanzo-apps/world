...
function downloadStory(): void {
  ...
  a.download = `world-${currentData?.countryCode.toLowerCase() || 'story'}-${Date.now()}.png`;
  a.click();
...
async function shareWhatsApp(data: StoryData): Promise<void> {
  ...
  const file = new File([currentBlob], `${data.countryCode.toLowerCase()}-world.png`, { type: 'image/png' });
