...
//     track floor (`--grid-cell`), so changing it re-snaps every panel. A faint
...
export const DEFAULT_CELL_SIZE = 120; // px — the grid track floor + free snap unit
...
function applyCellVar(): void {
  document.documentElement.style.setProperty('--grid-cell', `${state.cellSize}px`);
}
