...
export function generateStoryDeepLink(
  ...
  return `/v1/world/story?${params.toString()}`;
}
...
export const shareTexts = {
  ...
  linkedin: (data: StoryData) =>
    ...
    `\nData via Hanzo World - Open source geopolitical intelligence`,
...
export function getShareUrls(data: StoryData): Record<string, string> {
  ...
  return {
    ...
    reddit: `https://reddit.com/submit?url=${encodeURIComponent(url)}&title=${encodeURIComponent(`${data.countryName} Intelligence Brief - Hanzo World`)}`,
