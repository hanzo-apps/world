...
import { ingestProtestsForCII, ingestMilitaryForCII, ingestNewsForCII, ingestOutagesForCII, ingestConflictsForCII, ingestUcdpForCII, ingestHapiForCII, ingestDisplacementForCII, ingestClimateForCII, startLearning, isInLearningMode } from '@/services/country-instability';
...
export class App {
  ...
  constructor(containerId: string) {
    ...
    this.countryIntel = new CountryIntelController({
      getMap: () => this.map,
      getPanels: () => this.panels,
      getAllNews: () => this.allNews,
      ...
      buildAnalystHost: () => this.buildAnalystHost(),
    });
  }
  ...
  public async init(): Promise<void> {
    ...
    if (!this.isMobile) {
      ...
      this.findingsBadge.setOnSignalClick((signal) => {
        if (this.countryIntel.isBriefVisible()) return;
  ...
  private handleDeepLinks(): void {
    ...
    if (deepLinkCountry) {
      const cName = CountryIntelController.resolveCountryName(deepLinkCountry);
  ...
  private openCountryStory(code: string, name: string): void {
    this.countryIntel.openCountryStory(code, name);
  }

  // Read-only boot-readiness hook: window.__app.countryBriefPage stays valid
  // after the country-intel extraction (the page instance now lives in the
  // controller). See e2e/country-view.spec.ts's openCountry() wait.
  get countryBriefPage() {
    return this.countryIntel.getBriefPage();
  }
    ...
    this.container.innerHTML = `
      <div class="header">
        ...
        <div class="header-right">
          <button class="search-btn" id="searchBtn"><span class="search-ico" aria-hidden="true"><svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="7"></circle><path d="m20 20-3.4-3.4"></path></svg></span><kbd>⌘K</kbd> <span class="btn-label">${t('header.search')}</span></button>
  ...
  private getShareUrl(): string | null {
    ...
    return buildMapUrl(baseUrl, {
      ...
      layers: state.layers,
      country: this.countryIntel.isBriefVisible() ? (this.countryIntel.getBriefCode() ?? undefined) : undefined,
      mode: state.mode,
    });
  }
